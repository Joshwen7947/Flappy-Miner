...
class Miner(Main):
    ...
    def update(self):
        rotate_img = transform.scale(self.image, self.angle)
        screen.blit(rotate_img,(self.rect.x,self.rect.y))
...
display.set_caption("Flappy Miner 2.0")
clock = time.Clock()
...
pipes = [Pipe(WIDTH + 1 * 200,0,5) for i in range(2)]
